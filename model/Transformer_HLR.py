...
class TransformerModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_heads, encode_layers_num, output_dim=1, dropout=0.1,kernel_size = 3):
        ...
        #TODO
        #卷积层padding
        #self.conv = nn.Conv1d(in_channels=input_dim, out_channels=hidden_dim, kernel_size=kernel_size, stride=1, padding=kernel_size-1)
        self.conv = nn.Conv1d(in_channels=input_dim, out_channels=hidden_dim, kernel_size=kernel_size, stride=1, padding=0)
        self.kernel_size = kernel_size
        # 定义位置编码器
        self.positional_encoding = LearnablePositionalEncoding(hidden_dim, dropout=0)
    ...
    def forward(self, sample_tensor):
        ...
        tmp = nn.functional.pad(tmp, (self.kernel_size - 1, 0))
        # print(tmp.shape)
